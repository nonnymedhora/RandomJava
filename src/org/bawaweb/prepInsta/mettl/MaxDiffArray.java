/**
 * 
 */
package org.bawaweb.prepInsta.mettl;

/**
 * @author Navroz
 * 
 * https://prepinsta.com/mettl-coding-questions-1/
 * 
 * Maximum difference between two elements 
 * such that larger element appears after the smaller number
 * 
 * Given an array arr[] of integers,
 * find out the difference between any two elements 
 * such that larger element appears after the smaller number in arr[].

	Examples: If array is [2, 3, 10, 6, 4, 8, 1] 
	then returned value should be 8 (Diff between 10 and 2). 
	
	If array is [ 7, 9, 5, 6, 3, 2 ] 
	then returned value should be 2 (Diff between 7 and 9)

	Time Complexity: O(n^2)
	Auxiliary Space: O(1)

	Use two loops. In the outer loop, pick elements one by one and in the inner loop calculate the difference of the picked element with every other element in the array and compare the difference with the maximum difference calculated so far.

	Method 2 (Tricky and Efficient)
	In this method, instead of taking difference of the picked element with every other element, 
	we take the difference with the minimum element found so far. 
	So we need to keep track of 2 things:
	1) Maximum difference found so far (max_diff).
	2) Minimum number visited so far (min_element).

	Time Complexity: O(n)
	Auxiliary Space: O(1)
 *
 */
public class MaxDiffArray {
	
	
	public static int maxDiffArray(int[] arr) {
		if (arr == null || arr.length == 0)
			return 0;
		if (arr.length == 1)
			return arr[0];
		else {
			int minVal = arr[0];
			int maxDiff = 0;
			for (int i = 1; i < arr.length; i++) {
				if (arr[i] > minVal) {
					int diff = arr[i] - minVal;
					if (diff > maxDiff) {
						maxDiff = diff;
					}
				} else {
					minVal = arr[i];
				}
			}
			return maxDiff;
		}
	}
	
	public static void main(String[] args) {
		int[] anArr = new int[] { 2, 3, 10, 6, 4, 8, 1};
		System.out.println("For [2, 3, 10, 6, 4, 8, 1] maxDiffArray (from L --> R ) is ==> "+ maxDiffArray(anArr));
	
		int[] anArr1 = new int[] { 7, 9, 5, 6, 3, 2};
		System.out.println("For [7, 9, 5, 6, 3, 2] maxDiffArray (from L --> R ) is ==> "+ maxDiffArray(anArr1));
	}

}
